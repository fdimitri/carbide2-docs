...
| `srcpath` | string | Absolute DBFS path |
| `cur_name` | string | Current basename |
| `ftype` | string | `'file'` (default) or `'folder'` |
| `binary` | boolean | Binary-passthrough — reads go straight to DBFS disk |
...
- `project_settings` — one per project: DBFS flush tuning (`flush_bytes`,
