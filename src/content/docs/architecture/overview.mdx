...
Two long-lived processes (defined in the `Procfile`), plus a dev-only Vite
server and a shell sidecar backend:

| Process | Command | Purpose |
|---------|---------|---------|
| `rails` | `bundle exec rails server` | REST API + SPA asset serving |
| `worker` | `bundle exec ruby worker/worker.rb` | EventMachine WS server |
| Vite (dev only) | `npm run dev` | HMR for the Vue SPA |
| `carbide2-shell` (sidecar) | spawned/managed by the terminal backend | PTY exec target with a project-filesystem-only mount |
